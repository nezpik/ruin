    ...
    def step(self) -> None:
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if value <= 0:
                    continue
                retained = value * self.memory_weight * max(0.0, 1.0 - self.temporal_decay - self.recovery_rate)
                next_grid[y][x] += retained
                for yy in range(max(0, y - self.propagation_radius), min(self.height, y + self.propagation_radius + 1)):
                    for xx in range(max(0, x - self.propagation_radius), min(self.width, x + self.propagation_radius + 1)):
                        distance = abs(xx - x) + abs(yy - y)
                        if distance == 0 or distance > self.propagation_radius:
                            continue
                        next_grid[yy][xx] += value * (self.propagation_decay ** distance) * 0.1
        self.grid = next_grid
