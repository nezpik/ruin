...
import matplotlib
matplotlib.use("Agg")  # non-interactive backend for servers / tests
...
def save_text_frames(result: dict[str, Any], output: str | Path, limit: int = 10) -> None:
    """Legacy text-only frame exporter (kept for CLI compatibility)."""
    path = Path(output)
    lines: list[str] = []
...
def animate_probability_square(
    ...
    """
    Create an animated GIF of the Probability Square + Disruption Field.

    Uses blitting for reasonable performance on moderate grids (30x30 is fine).
    """
    ...
    # Prepare figure (dark theme for "ruin" aesthetic)
    ...
    # Scatter for Q-dots (we'll update colors/sizes)
    ...
    def update(frame_idx: int):
        ...
        # Field: synthetic for v0.2 (full grid storage coming later)
        ...
        field_img.set_clim(0, max(2.0, field.max() * 1.1))

        # Q-dots — position is tuple (x, y), ruined flag is "ruined"
        ...
        if qdots:
            ...
            for q in qdots:
                if q.get("ruined"):
                    colors.append(0.0)   # ruined = dark
                    sizes.append(18)
                elif q.get("late"):
                    colors.append(0.3)   # late = orange-ish
                    sizes.append(28)
                else:
...
def save_field_gif(
    ...
    """Convenience wrapper for animate_probability_square → GIF."""
